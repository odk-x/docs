...
There is an important library called :dfn:`Sync Client` which implements the sync protocol by interacting with Sync endpoint and other tools. The source of this library can be found here: `https://github.com/odk-x/sync-client <https://github.com/odk-x/sync-client/>`_. This is a libary that can be used to execute the ODK 2.0 sync protocol natively on non-Android systems.
